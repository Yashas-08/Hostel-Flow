...
export default function EditProfilePage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setSaving(true);
    try {
      await api.patch<StudentProfile>('/students/me', {
        phone: phone.trim() || null,
        avatarUrl: avatarPreview === undefined ? undefined : avatarPreview,
      });
      navigate('/student/profile', { replace: true, state: { profileUpdated: true } });
    } catch (err) {
        ...
        <form onSubmit={handleSubmit} className="space-y-5" noValidate>
          ...
          <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[var(--radius-lg)] p-4 space-y-1">
            <p className="text-xs text-[var(--color-ink-muted)]">
              Full name, student ID, course, year, and room details are managed by the hostel
              admin and can't be edited here.
            </p>
          </div>
